    ...
    public func run(checkProtocolTooling: Bool, checkNativeDependencies: Bool) throws {
        ...
        for path in required {
            ...
            guard context.fileSystem.exists(url) else {
                throw ToolError("missing required repository path: \(path)", exitCode: ToolExitCode.environment)
            }
...
public struct BootstrapChecker {
    ...
    public func check(maintainer: Bool = false) throws {
        ...
        _ = try context.swift.runSwift(["package", "describe", "--type", "json"], repository: context.repository)
        context.diagnostics.success("SwiftPM")
        _ = try context.runner.executableURL(for: "pkg-config")
        context.diagnostics.success("pkg-config")

        for module in ["wayland-client", "wayland-server", "wayland-cursor", "wayland-egl", "xkbcommon", "libdrm", "gbm", "egl", "glesv2"] {
            ...
            guard result.exitCode == 0 else {
                throw ToolError("missing pkg-config module: \(module)", exitCode: ToolExitCode.environment)
            }
        ...
        for tool in ["wayland-scanner", "weston", "swiftlint"] {
            if context.runner.canFind(tool) {
                ...
                context.diagnostics.warning("\(tool) is not installed; related checks will fail when requested")
            } else {
                throw ToolError("missing required tool: \(tool)", exitCode: ToolExitCode.environment)
            }
        }
        ...
        if maintainer {
            ...
            ).resolvedSources().forEach { entry, source in
                guard let source else {
                    throw ToolError("missing protocol source for \(entry.name)", exitCode: ToolExitCode.environment)
                }
    ...
    public func installCommand(packageManager: String) throws -> String {
        guard let packages = BootstrapPackages.packages[packageManager] else {
            throw ToolError("unsupported package manager: \(packageManager)", exitCode: ToolExitCode.usage)
        }
    ...
    private func canResolveSwiftFormat() -> Bool {
        ...
        let result = try? context.runner.run("swift", ["format", "--version"], requireSuccess: false)
...
public enum BootstrapPackages {
    public static let packages: [String: [String]] = [
        "apt-get": ["clang", "git", "libdrm-dev", "libegl-dev", "libgbm-dev", "libgles-dev", "libwayland-dev", "libxkbcommon-dev", "pkg-config", "ripgrep", "wayland-protocols", "weston", "curl", "unzip", "just"],
        "dnf": ["clang", "git", "libdrm-devel", "mesa-libEGL-devel", "mesa-libgbm-devel", "mesa-libGLES-devel", "wayland-devel", "wayland-protocols-devel", "libxkbcommon-devel", "pkgconf-pkg-config", "ripgrep", "weston", "curl", "unzip", "just"],
        "pacman": ["clang", "git", "libdrm", "mesa", "wayland", "wayland-protocols", "libxkbcommon", "pkgconf", "ripgrep", "weston", "curl", "unzip", "just"],
        "zypper": ["clang", "git", "libdrm-devel", "Mesa-libEGL-devel", "libgbm-devel", "Mesa-libGLESv2-devel", "wayland-devel", "wayland-protocols-devel", "libxkbcommon-devel", "pkgconf-pkg-config", "ripgrep", "weston", "curl", "unzip", "just"],
        "apk": ["clang", "git", "libdrm-dev", "mesa-dev", "wayland-dev", "wayland-protocols", "libxkbcommon-dev", "pkgconf", "ripgrep", "weston", "curl", "unzip", "just"],
        "emerge": ["sys-devel/clang", "dev-vcs/git", "x11-libs/libdrm", "media-libs/mesa", "dev-libs/wayland", "dev-libs/wayland-protocols", "dev-util/wayland-scanner", "x11-libs/libxkbcommon", "virtual/pkgconfig", "sys-apps/ripgrep", "gui-libs/weston", "net-misc/curl", "app-arch/unzip"],
        "nix": [],
    ]
}
...
public struct SwiftCommandResolver {
    ...
    public func runSwift(_ arguments: [String], environment: [String: String] = [:]) throws {
        try context.swift.runSwift(arguments, repository: context.repository, environment: environment)
    }
    ...
    public func runSwiftFormat(_ arguments: [String]) throws {
        ...
        if context.runner.canFind("swift-format") {
            try context.runner.run("swift-format", arguments, workingDirectory: context.repository.root)
            return
        }
        try context.runner.run("swift", ["format"] + arguments, workingDirectory: context.repository.root)
    }
    ...
    public func runSwiftLint() throws {
        let args = ["lint", "--strict", "--no-cache", "--force-exclude", "--config", ".swiftlint.yml"]
...
public struct VerificationChecks {
    ...
    public func verifyTargetImports() throws {
        let rules: [(String, String, [String])] = [
            ("WaylandRaw", "Sources/WaylandRaw", ["WaylandClient", "WaylandKeyboard", "WaylandCursor", "WaylandGraphicsCore", "WaylandGraphicsPreview"]),
        ...
        for (label, path, modules) in rules {
            ...
            for file in try context.fileSystem.walk(root, includingDirectories: false).filter({ $0.pathExtension == "swift" }) {
                let text = try context.fileSystem.readText(file)
                for (index, line) in text.split(separator: "\n", omittingEmptySubsequences: false).enumerated() {
                    for module in modules where line.range(of: #"^\s*import\s+\#(module)\b"#, options: .regularExpression) != nil {
                        failures.append("\(context.repository.relativePath(file)):\(index + 1): \(label) must not import \(module)")
                    }
                }
            }
        }
    ...
    public func verifyUnsafeAllowlist() throws {
        ...
        for root in roots {
            ...
            for file in files where file.pathExtension == "swift" || file.lastPathComponent == "Package.swift" {
                let text = try context.fileSystem.readText(file)
                ...
                for (index, line) in lines.enumerated() {
                    ...
                    for match in regex.matches(in: line, range: nsRange) {
                        ...
                        if token.hasPrefix("@unchecked") && !hasSafetyComment(lines: lines, lineIndex: index) {
                            failures.append("\(relative):\(index + 1): @unchecked Sendable allowlist entry requires a nearby SAFETY comment")
                        }
                    }
                }
            }
    ...
    public func verifyReleaseShimSymbols() throws {
        try context.swift.runSwift(["build", "--disable-index-store", "-c", "release", "--target", "CWaylandProtocols"], repository: context.repository)
        try context.swift.runSwift(["build", "--disable-index-store", "-c", "release", "--target", "CGBMShims"], repository: context.repository)
        _ = try context.runner.executableURL(for: "nm")
        let objects = try context.fileSystem.walk(context.repository.url(".build"), includingDirectories: false)
            .filter { url in
                url.pathExtension == "o"
                    && (url.path.contains("/release/CWaylandProtocols.build/") || url.path.contains("/release/CGBMShims.build/"))
            }
        ...
        for object in objects {
            let result = try context.runner.run("nm", ["-g", object.path], workingDirectory: context.repository.root)
...
public struct PublicAPIAuditor {
    ...
    public func dump() throws -> String {
        ...
        """
        let package = try context.swift.runSwift(["package", "describe", "--type", "json"], repository: context.repository)
        ...
        output += "\n"
        output += try declarationsSection(title: "WaylandClient Public Declarations", rootPath: "Sources/WaylandClient")
        output += try declarationsSection(title: "WaylandGraphicsPreview Public Declarations", rootPath: "Sources/WaylandGraphicsPreviewAPI")
    ...
    public func verify(update: Bool) throws {
        ...
        guard context.fileSystem.exists(baseline) else {
            throw ToolError("Missing public API baseline: docs/public-api-baseline.md", exitCode: ToolExitCode.data)
        }
        let current = try context.fileSystem.readText(baseline)
        guard current == baselineText else {
            throw ToolError("SwiftWayland public API changed; review and update docs/public-api-baseline.md", exitCode: ToolExitCode.data)
        }
    ...
    private func declarationsSection(title: String, rootPath: String) throws -> String {
        var output = "## \(title)\n\n"
        let files = try context.fileSystem.walk(context.repository.url(rootPath), includingDirectories: false)
    ...
    private func nonProductDeclarations() throws -> String {
        ...
        output += "These declarations are not part of a vended library product unless the package manifest changes.\n\n"
        let excluded = ["Sources/WaylandClient/", "Sources/WaylandGraphicsPreviewAPI/"]
        let files = try context.fileSystem.walk(context.repository.url("Sources"), includingDirectories: false)
            .filter { $0.pathExtension == "swift" }
            .filter { file in
                let relative = context.repository.relativePath(file)
    ...
    private func publicDeclarations(in file: URL) throws -> [(line: Int, text: String)] {
        let lines = try context.fileSystem.readText(file).split(separator: "\n", omittingEmptySubsequences: false).map(String.init)
        ...
        for (index, line) in lines.enumerated() {
            if line.range(of: #"^\s*([A-Za-z_][A-Za-z0-9_]*\s+)*public\s+"#, options: .regularExpression) != nil {
                result.append((index + 1, line))
            }

            let startsPublicEnum = line.range(of: #"^\s*([A-Za-z_][A-Za-z0-9_]*\s+)*public\s+(indirect\s+)?enum\s+"#, options: .regularExpression) != nil
            if startsPublicEnum {
                inPublicEnum = true
                enumDepth = 0
                enumSeenBody = false
            } else if inPublicEnum && enumSeenBody && enumDepth == 1
    ...
    private func extractBaseline(from report: String) -> String {
        ...
        var include = false
        for line in lines {
            if line == "## WaylandClient Public Declarations" || line == "## WaylandGraphicsPreview Public Declarations" {
                include = true
...
public struct HeadlessWestonRunner {
    ...
    public func run(command: [String], timeoutSeconds: TimeInterval = 600) throws {
        ...
        let runtime = try context.fileSystem.createTemporaryDirectory(prefix: "swiftwayland-runtime")
    ...
    private func waitForSocket(_ socket: URL, process: Process) throws {
        for _ in 0..<100 {
            ...
            if !process.isRunning {
                throw ToolError("weston exited before creating \(socket.path)", exitCode: ToolExitCode.process)
            }
    ...
    private func runChild(command: [String], environment: [String: String], timeoutSeconds: TimeInterval) throws {
        let process = Process()
        ...
        if process.isRunning {
            process.terminate()
            throw ToolError("headless command timed out: \(command.joined(separator: " "))", exitCode: ToolExitCode.process)
        }
        guard process.terminationStatus == 0 else {
            throw ToolError(
                "headless command failed with exit code \(process.terminationStatus): \(command.joined(separator: " "))",
