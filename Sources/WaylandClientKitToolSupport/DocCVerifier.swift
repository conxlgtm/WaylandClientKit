...
public struct DocCVerifier {
    ...
    public func requirePublicProductSymbolGraphs(
        afterDump result: ProcessResult,
        allowingNonProductFailures: Bool = false
    ) throws {
        for product in Self.publicProducts {
            _ = try requireSymbolGraph(
                for: product,
                afterDump: result,
                allowingDumpFailure: allowingNonProductFailures
            )
        }
        if allowingNonProductFailures, result.exitCode != 0 {
            diagnostics.warning(
                "dump-symbol-graph reported non-product failures "
                    + "after emitting public product symbol graphs"
            )
        }
    }
    ...
    private func requireSymbolGraph(
        ...
        if result.exitCode != 0, !allowingDumpFailure {
            throw Self.symbolGraphDumpError(
                result,
                detail:
