...
static void swl_registry_handle_global(
    ...
    uint32_t version)
{
    const struct swl_registry_listener_callbacks *callbacks = data;
    if (callbacks != NULL && callbacks->global != NULL)
    {
        callbacks->global(callbacks->data, registry, name, interface, version);
    }
}
...
{
    const struct swl_registry_listener_callbacks *callbacks = data;
    if (callbacks != NULL && callbacks->global_remove != NULL)
    {
        callbacks->global_remove(callbacks->data, registry, name);
    }
}
...
{
    const struct swl_callback_listener_callbacks *callbacks = data;
    if (callbacks != NULL && callbacks->done != NULL)
    {
        callbacks->done(callbacks->data, callback, callback_data);
    }
}
    ...
    const struct swl_callback_listener_callbacks *callbacks)
{
    return wl_callback_add_listener(
        callback,
        &swl_callback_listener_impl,
        (void *)callbacks);
}
/*
 * wl_buffer listener bridge
 */
static void swl_buffer_handle_release(
    void *data,
    struct wl_buffer *buffer)
{
    const struct swl_buffer_listener_callbacks *callbacks = data;
    if (callbacks != NULL && callbacks->release != NULL)
    {
        callbacks->release(callbacks->data, buffer);
    }
}
