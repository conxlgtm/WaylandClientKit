...
| `WaylandGraphicsExternalBufferSubmissionReceipt` | typed foreign-submission error when persistent registration lands | typed unknown-submission error when persistent registration lands | waiter resolves `backingClosed` | waiter resolves `backingClosed` | n/a | receipt reaches one terminal release result | repeated wait returns same result | late release after backing close is ignored |
