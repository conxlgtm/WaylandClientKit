...
| `WaylandGraphicsExternalBuffer` | `foreignExternalBuffer` | `externalBufferUnavailable` | registration/reservation fails closed | backing close retires registrations | n/a | buffer retired with backing | duplicate reservation fails while busy | late release after backing close is ignored |
