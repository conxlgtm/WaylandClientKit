...
| `WaylandGraphicsExternalBuffer` | `foreignExternalBuffer` | `externalBufferUnavailable` | registration/reservation fails closed | backing close retires registrations | n/a | unregister or backing close retires registration | duplicate reservation fails while busy, duplicate unregister is unavailable | late release after backing close is ignored |
| `WaylandGraphicsExternalBufferSubmissionReceipt` | n/a | n/a | waiter resolves `backingClosed` | waiter resolves `backingClosed` | n/a | receipt reaches one terminal release result | repeated wait returns same result | late release after backing close is ignored |
| `WaylandGraphicsExternalSyncTimeline` | typed unavailable when used across backing | typed unavailable when not imported | import fails closed | backing close retires timeline mapping | n/a | compositor owns imported timeline proxy | duplicate point use is renderer policy, WCK validates imported identity | late release signal after backing close is ignored |
