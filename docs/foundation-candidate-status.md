...
- Explicit sync and commit-timing active evidence on real compositors. FIFO has
  active KDE/KWin evidence; explicit sync currently reports a typed
  `explicitSyncReleaseFailed` live failure on KDE/KWin, and commit timing
  reports typed fallback where unavailable.
