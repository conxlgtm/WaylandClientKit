...
| GNOME / Mutter | environment skip(GNOME/Mutter Wayland session or VM unavailable on 2026-06-09 host) | environment skip(GNOME/Mutter Wayland session unavailable) | environment skip(GNOME/Mutter Wayland session unavailable) | environment skip(GNOME/Mutter Wayland session unavailable) | environment skip(GNOME/Mutter Wayland session unavailable) | Real desktop target still needed for foundation readiness. |
| KDE / KWin | KDE / plasma session, 2026-06-09 | `wayland-info`: dmabuf v5, linux-drm-syncobj v1, FIFO v1, presentation v2, text-input v3 v1, cursor-shape v2, pointer constraints v1, relative pointer v1, top-level icon v1, idle inhibit v1, system bell v1, xdg activation v1, color metadata advertised, commit timing unavailable | `swift run swl smoke live`, `swift run swl smoke integration`, `swift run swl smoke gpu-preview`, and individual auto-close feature examples passed | live integration smoke passed; `swift run swl ci check` was attempted but hung after building `swl` with no child process/output | `GPUPreviewSmokeClient` and `GraphicsPreviewManagedGPUClear` reported active managed GPU submission | Active GPU presentation proven for clear-frame submission on this run. Manual serial, pointer-capture motion, data-transfer drag-source, and GPU resize interactions remain unproven. |
