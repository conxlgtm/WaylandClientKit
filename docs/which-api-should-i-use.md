...
| Track external GPU buffer preview status | `WaylandGraphicsFrameLease.submitExternalBuffer` with an opaque preview descriptor | `zwp_linux_dmabuf_v1`; FD-backed descriptor construction is maintainer-only until a raw-handle-free public handoff exists | [External buffer docs](../Sources/WaylandGraphicsPreviewAPI/WaylandGraphicsPreview.docc/ExternalBufferSubmission.md) |
