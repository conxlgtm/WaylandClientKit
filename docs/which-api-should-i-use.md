...
WaylandClientKit is the substrate layer. This table maps common Wayland-facing
tasks to public APIs and examples.
...
| Draw pixels on a managed surface | `Window`, `PopupSurface`, or `Subsurface` `show`/`redraw` with `SoftwareFrame` | `wl_shm` plus the role's lifecycle gate | [ManagedSurfacePresentationSmoke](../Examples/ManagedSurfacePresentationSmoke/ManagedSurfacePresentationSmoke.swift) |
...
| Redraw part of a managed surface | `SurfaceFrameMetadata(damage:)` with the role's `redraw` method | surface size and scale must validate | [Managed surface drawing](../Sources/WaylandClient/WaylandClient.docc/ManagedSurfaceDrawing.md) |
...
| Create and present a child surface | `Window.createSubsurface`, `Subsurface.show`, `Subsurface.redraw` | `wl_subcompositor` | [ManagedSurfacePresentationSmoke](../Examples/ManagedSurfacePresentationSmoke/ManagedSurfacePresentationSmoke.swift) |
| Create and present an xdg popup | `Window.createPopup`, `PopupSurface.show`, `PopupSurface.redraw` | `xdg_wm_base` popup configure | [ManagedSurfacePresentationSmoke](../Examples/ManagedSurfacePresentationSmoke/ManagedSurfacePresentationSmoke.swift) |
...
| Inspect compositor session support | `WaylandDisplay.capabilities().compositorSessionManagement` | `xdg_session_manager_v1`, no public session handle yet | [CompositorSessionSmoke](../Examples/CompositorSessionSmoke/main.swift) |
...
| Observe real presentation for any managed role | `requestPresentationFeedback: true`, each handle's `presentationEvents`, and `DisplayEvent.presentation` | `wp_presentation` | [ManagedSurfacePresentationSmoke](../Examples/ManagedSurfacePresentationSmoke/ManagedSurfacePresentationSmoke.swift) |
