...
struct RawSeatLifecycleTests {  // swiftlint:disable:this type_body_length
    ...
    func capabilityChangesCreateAndReleaseChildProxies() throws {
        ...
        unsafe recorder.pointerProxy = fakePointer(0x201)
        unsafe recorder.keyboardProxy = fakePointer(0x202)
    ...
    func childCreationFailureKeepsAdvertisedStateAndCreatesOtherChildren() throws {
        let recorder = SeatOperationRecorder()
        unsafe recorder.pointerProxy = nil
        unsafe recorder.keyboardProxy = fakePointer(0x302)
        let queue = RawInputEventQueue()
        let seat = try unsafe RawSeat(
    ...
    func destroyAfterChildCreationFailureReleasesOnlyCreatedChildren() throws {
        let recorder = SeatOperationRecorder()
        unsafe recorder.pointerProxy = nil
        unsafe recorder.keyboardProxy = fakePointer(0x312)
        let queue = RawInputEventQueue()
        let seat = try unsafe RawSeat(
    ...
    func pointerListenerInstallFailureEmitsSeatBindingDiagnostic() throws {
        let recorder = SeatOperationRecorder()
        unsafe recorder.pointerProxy = fakePointer(0x331)
    ...
    func keyboardListenerInstallFailureEmitsSeatBindingDiagnostic() throws {
        let recorder = SeatOperationRecorder()
        unsafe recorder.keyboardProxy = fakePointer(0x341)
    ...
    func globalRemovalDestroysChildrenBeforeSeatAndEmitsRemoval() throws {
        let recorder = SeatOperationRecorder()
        unsafe recorder.pointerProxy = fakePointer(0x401)
        unsafe recorder.keyboardProxy = fakePointer(0x402)
        unsafe recorder.touchProxy = fakePointer(0x403)
        let queue = RawInputEventQueue()
        let seat = try unsafe RawSeat(
    ...
    func nullNameCallbackRecordsRawInvariantFailure() throws {
        ...
        let seat = try unsafe RawSeat(
            ...
            pointer: try unsafe #require(fakePointer(0x502)),
            version: 10,
            eventSink: queue,
    ...
    func pointerCallbacksEmitInputEvents() throws {
        let recorder = SeatOperationRecorder()
        unsafe recorder.pointerProxy = fakePointer(0x801)
        let queue = RawInputEventQueue()
        let seat = try unsafe RawSeat(
        ...
        try seat.applyCapabilities([.pointer])
        _ = queue.drain()
        let callbacks = try unsafe #require(recorder.pointerCallbacks)
        unsafe callbacks.pointee.motion?(
            callbacks.pointee.data,
            unsafe recorder.pointerProxy,
            123,
            256,
            512
        )
    ...
    func setPointerCursorUsesCurrentPointerChild() throws {
        let recorder = SeatOperationRecorder()
        unsafe recorder.pointerProxy = fakePointer(0x8A1)
        let queue = RawInputEventQueue()
        let seat = try unsafe RawSeat(
        ...
        _ = queue.drain()
        recorder.entries.removeAll()
        let result = unsafe seat.setPointerCursor(
            serial: 77,
    ...
    func keyboardCallbacksCopyEnterKeys() throws {
        let recorder = SeatOperationRecorder()
        unsafe recorder.keyboardProxy = fakePointer(0x901)
        let queue = RawInputEventQueue()
        let seat = try unsafe RawSeat(
        ...
        try seat.applyCapabilities([.keyboard])
        _ = queue.drain()
        let callbacks = try unsafe #require(recorder.keyboardCallbacks)
        var pressedKeys = [UInt32(30), UInt32(31)]
        let keyArrayByteCount = pressedKeys.count * MemoryLayout<UInt32>.stride
        unsafe pressedKeys.withUnsafeMutableBufferPointer { keyBuffer in
    ...
    func keyboardKeymapCallbackCopiesBytes() throws {
        let recorder = SeatOperationRecorder()
        unsafe recorder.keyboardProxy = fakePointer(0x981)
        let queue = RawInputEventQueue()
        let seat = try unsafe RawSeat(
        ...
        unsafe callbacks.pointee.keymap?(
            callbacks.pointee.data,
            unsafe recorder.keyboardProxy,
    ...
    func touchCallbacksEmitInputEvents() throws {
        let recorder = SeatOperationRecorder()
        unsafe recorder.touchProxy = fakePointer(0xA01)
        let queue = RawInputEventQueue()
        let seat = try unsafe RawSeat(
        ...
        _ = queue.drain()
        let callbacks = try unsafe #require(recorder.touchCallbacks)
        unsafe callbacks.pointee.down?(
            callbacks.pointee.data,
            unsafe recorder.touchProxy,
            1,
            2,
            fakePointer(0x333),
            3,
    ...
    var operations: RawSeatProxyOperations {
        unsafe RawSeatProxyOperations(
            ...
            proxyObjectID: { proxy in
                ...
                let rawPointer = unsafe UnsafeRawPointer(proxy)
                return RawObjectID(UInt32(UInt(bitPattern: rawPointer)))
            },
...
private func makeKeyboardSeatFixture(
    ...
    let recorder = SeatOperationRecorder()
    unsafe recorder.keyboardProxy = fakePointer(keyboardPointer)
    let queue = RawInputEventQueue()
    let seat = try unsafe RawSeat(
        id: seatID,
    ...
    try seat.applyCapabilities([.keyboard])
    _ = queue.drain()
    return unsafe KeyboardSeatFixture(
        seat: seat,
        queue: queue,
        callbacks: try unsafe #require(recorder.keyboardCallbacks),
        keyboardProxy: unsafe recorder.keyboardProxy
    )
}
private func sendKeyboardKeymap(
    ...
    let descriptor = try makeTemporaryFileDescriptor(bytes: bytes)
    unsafe fixture.callbacks.pointee.keymap?(
...
private func fakePointer(_ bitPattern: Int) -> OpaquePointer? {
    unsafe OpaquePointer(bitPattern: bitPattern)
}
