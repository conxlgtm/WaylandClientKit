...
struct DocCVerifierProductTests {
    ...
    @Test
    func doccVerifierCanIgnoreNonProductDumpFailures() throws {
        let root = try temporaryRepository()
        let scratch = root.appendingPathComponent(".scratch")
        ...
        try #"{"symbols":[]}"#.write(to: clientGraph, atomically: true, encoding: .utf8)
        try #"{"symbols":[]}"#.write(to: previewGraph, atomically: true, encoding: .utf8)
        ...
        let result = ProcessResult(
            ...
            exitCode: 1,
            stdout: "",
            stderr: "failed package test module")
        let verifier = DocCVerifier(repository: Repository(root: root), buildRoot: scratch)
    ...
    private func temporaryRepository() throws -> URL {
        let root = URL(fileURLWithPath: NSTemporaryDirectory())
            .appendingPathComponent("waylandclientkit-docc-product-tests-\(UUID().uuidString)")
