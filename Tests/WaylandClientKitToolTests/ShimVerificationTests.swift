...
struct ShimVerificationTests {
    ...
    func shimVerificationRejectsGBMHeaderTestingDefaults() throws {
        ...
        do {
            ...
            #expect(error.message.contains("wayland-client-kit-gbm-shims.h"))
        }
    }

    private func temporaryRepository() throws -> URL {
        let root = URL(fileURLWithPath: NSTemporaryDirectory())
            .appendingPathComponent("waylandclientkit-shim-verification-tests-\(UUID().uuidString)")
    ...
    private func writeRequiredHeaders(in root: URL) throws {
        ...
        try writeFile(
            ...
            text: "")
        try writeFile(
            root.appendingPathComponent("Sources/CGBMShims/include/wayland-client-kit-gbm-shims.h"),
            text: "")
    }
